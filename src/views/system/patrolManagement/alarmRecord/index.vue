  ...
  <div class="page">
    <div class="search-content">
      <el-form class="queryForm" ref="queryFormRef" :model="queryParams" label-width="auto" :inline="true"
        ...
        <el-form-item label="真实姓名:" label-width="84px">
          <el-input v-model="queryParams.user_name" placeholder="请输入真实姓名" clearable></el-input>
        </el-form-item>
        <el-form-item label="类别：">
          <el-select style="width: 200px;" v-model="queryParams.alarmtype" :teleported="false" placeholder="请选择"
            clearable>
    ...
    <div class="table-box">
      <el-table class="tableList" v-adaptive="{ bottom: 68 }" :data="tableData">
        ...
        <el-table-column prop="collectionname" label="报警名称" align="center">
        </el-table-column>
        <el-table-column prop="alarmtypename" label="报警类型" align="center">
        </el-table-column>
        <el-table-column prop="real_name" label="报警人" align="center">
        </el-table-column>
        <el-table-column prop="remark" label="报警地址" show-overflow-tooltip align="center">
        </el-table-column>
        <el-table-column prop="locationtype" label="定位类型" align="center">
        </el-table-column>
        <el-table-column prop="real_lon" label="经度" align="center"> </el-table-column>
        <el-table-column prop="real_lat" label="纬度" align="center"> </el-table-column>
        <el-table-column prop="uploaddate" label="报警时间" align="center">
        </el-table-column>
...
const getList = (val?: number) => {
  ...
  alarmApi.getDataByPage(queryParams.value).then((res: any) => {
    if (res.success) {
...
const handleDelete = (id: number) => {
  ElMessageBox.confirm("是否确认删除此条数据?", "提示", {
    ...
    .then(() => {
      alarmApi.delete({ id: id }).then((res: any) => {
        if (res.success) {
          ElMessage({
            type: "success",
            message: "删除成功",
