    ...
    <div class="page">
        ...
        <div class="table-box">
            <el-table class="tableList" v-adaptive="{ bottom: 68 }" v-loading="loading" :data="tableList">
                ...
                <el-table-column prop="mc" label="名称" align="left"> </el-table-column>
                <el-table-column prop="bh" label="编号" align="left"></el-table-column>
                <el-table-column prop="lon" label="经度" align="left"></el-table-column>
                <el-table-column prop="lat" label="纬度" align="left"></el-table-column>
                <el-table-column prop="settime" label="放置时间" align="left" />
                <el-table-column prop="paybacktime" label="回收时间" align="left" />
                <el-table-column prop="dlstate" label="设备状态" align="left">
                    <template #default="scope">
                        <el-tag>{{ scope.row.dlstate }}</el-tag>
                    </template>
                </el-table-column>
...
const getList = (val?: number) => {
    ...
    infraredApi.getData(queryParams.value).then((res) => {
        if (res.success) {
...
const handleDelete = (id: number) => {
    ElMessageBox.confirm("是否确认删除此条数据?", "提示", {
        ...
        .then(() => {
            infraredApi.delete({ imageId: id}).then((res: any) => {
                if (res.success) {
                    ElMessage({
                        type: "success",
                        message: "删除成功",
