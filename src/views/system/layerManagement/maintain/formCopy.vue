            ...
            <el-form ref="formRef" v-loading="loadForm" :model="form" :inline="true" label-position="left"
                ...
                <template v-if="form.type != '文件夹'">
                    ...
                    <el-form-item v-if="optionsField.length > 0" label="弹出框字段：" prop="fields"
                        ...
                        <el-select v-model="form.fields" :teleported="false" placeholder="请选择" multiple collapse-tags
                            collapse-tags-tooltip :max-collapse-tags="5" style="width: 100%;">
                            <el-option v-for="(item, index) in optionsField" :key="index" :label="item.fieldCNName"
                                :value="item.fieldName">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <!-- <el-form-item label="定位图标：" v-if="form.type == '点' || form.type == '本底资源'" prop="icon"
                        :rules="{ required: true, message: '不能为空', trigger: 'change' }">
                        <UploadImg v-model="form.icon"></UploadImg>
                    </el-form-item> -->
                    <el-form-item label="边框颜色：" v-if="form.type == '线' || form.type == '面'" prop="frame"
                        :rules="{ required: true, message: '不能为空', trigger: 'change' }">
                        <el-color-picker v-model="form.frame" />
                    </el-form-item>
                    <el-form-item label="填充颜色：" v-if="form.type == '面'" prop="fill"
                        :rules="{ required: false, message: '不能为空', trigger: 'change' }">
                        <el-color-picker v-model="form.fill" show-alpha />
                    </el-form-item>
                    <!-- 保护区界线 -->
                    <div v-if="form.type == '保护区界线'" v-for="(item, index) in layerFields" :key="index + 100"
                        style="width: 100%;display: flex;flex-wrap: wrap;justify-content: space-between;align-items: center;">
                        <el-form-item label="名称" style="width: 30%;">
                            <el-input v-model="item.name" placeholder="请输入" :disadled="true"></el-input>
                        </el-form-item>
                        <el-form-item label="填充颜色" v-if="item.type == '面'" prop="'popuFields.' + index + '.fill'"
                            style="width: 30%;">
                            <el-color-picker v-model="item.fill" show-alpha :key="index" />
                        </el-form-item>
                        <el-form-item label="边框颜色" v-if="item.type == '线' || item.type == '面'" style="width: 30%;">
                            <el-color-picker v-model="item.frame" :key="index" />
                        </el-form-item>
                    </div>
                </template>
            ...
            if (res.success) {
                ...
                if (form.value.tableName) {
                    ...
                    if (res.response.text) {
                        layerFields.value = JSON.parse(res.response.text)
                    } else {
                        layerFields.value = [{
                            name: '界限',
                            type: '线',
