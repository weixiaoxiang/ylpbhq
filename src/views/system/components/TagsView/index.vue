    ...
    >
      <transition-group name="el-zoom-in-center">
        <router-link
          ...
          :class="{ active: activeTag.fullPath === tag.fullPath }"
          :to="{ path: tag.fullPath, query: tag.query }"
          @contextmenu.prevent="openMenu(tag, $event)"
        >
          <el-icon size="14">
            <SvgIcon :name="tag?.icon || '默认_default'" />
          </el-icon>
          {{ tag?.title }}
          <el-icon
            v-if="!tag?.affix"
...
.tags-view-container {
  ...
  .tags-view-wrapper {
    ...
    .tags-view-item {
      ...
      border-radius: 2px;
      margin: 0 3px;
      color: var(--system-tagsview-tag-text-color);
      background-color: var(--system-tagsview-tag-bg-color);
      transition: all 0.3s;
      padding: 0 5px;
      font-size: 12px;
      &:first-of-type {
      ...
      &.active {
        background-color: var(--system-tagsview-tag-active-bg-color);
        color: var(--system-tagsview-tag-active-text-color);
        border-color: var(--system-tagsview-tag-active-border-color);
      }
      .el-icon {
        margin: 0 2px;
