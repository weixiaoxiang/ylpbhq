...
const viewImg = () => {
  if (props.detail.imgsrcList.length > 0) {
    u.showPreviewImg(
      0,
      props.detail.imgsrcList.map((item: string) => `/fireStaticUrl/${item}`)
    )
  } else {
...
const queryReplay = async (time?: string) => {
  ...
  try {
    ...
    const { response } = await GetPlaybackURL({
      deviceId: "f3eae95f5045419aae6474a62850d79c", // props.detail.monithorcode, // "f3eae95f5045419aae6474a62850d79c",
      Playtype: "ws",
      beginTime: startTime,
      endTime: endTime
    })
        ...
        <div class="con">
          ...
          <div class="info-box">
            ...
            <div class="info-item">
              ...
              <div class="value">{{ detail?.height || "暂无" }}</div>
            </div>
            <div class="info-item">
          ...
          >
            ...
            >
              <el-image
                :src="detail?.imgsrcList?.[0] ? '/fireStaticUrl/' + detail?.imgsrcList?.[0] : ''"
                fit="cover"
                @click="viewImg"
              >
          ...
          <div class="lzjl-box">
            ...
            <div class="row yptime">
              ...
              <div class="user">研判人：{{ detail?.judgeuser }}</div>
            </div>
