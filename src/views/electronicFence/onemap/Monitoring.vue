...
import { GetElectronicfenceEquipmentTreeList, GetMmonitoringCj, GetPlaybackURL, GetPtzControl } from "@/api"
...
// 获得监控树
const getData = async () => {
  const { response } = await GetElectronicfenceEquipmentTreeList()
  response.forEach((item: any) => {
    ...
    item.ChildModel = item.list
      .filter((child: any) => child.name)
      .map((child: any) => {
        child.label = child.name
        child.eqtypename = child.eqtypename?.slice(0, 3)
        // todo:先写死，等待对接
        // child.devicenumber = "509b4ac31ef24a2d97a69f2561fc603f"
        return child
      })
    delete item.list
    delete item.name
  ...
  try {
    ...
    if (data.ChildModel) {
      data.ChildModel.forEach((item: any) => {
        if (item.eqtypename.indexOf("增强型") > -1) {
          ElMessage.info("增强型设备不支持播放！")
          return
        }
        if (item.status !== "在线" || !item.label) {
          ElMessage.info("当前设备离线，无法播放！")
          return
        }
        promiseAll.push(
          axios.get(
            "https://gwdm.510link.com/api/devices/dCameraInfoVServerApi/liveStart?uniqueCode=" + item.equipmentId,
            {
              headers: {
      ...
      if (data.eqtypename.indexOf("增强型") > -1) {
        ElMessage.info("增强型设备不支持播放！")
        return
      }
      if (data.status !== "在线") {
      ...
      promiseAll.push(
        axios.get(
          "https://gwdm.510link.com/api/devices/dCameraInfoVServerApi/liveStart?uniqueCode=" + data.equipmentId,
          {
            headers: {
              "X-MAAAPI-ApiKey": "lOvA9FzlTYkOx4cg98UOTA==",
              "X-MAAAPI-ApiSecret": "pyNB9Ld11sn3ttidM+I9cUsOGSvYUep2G73lZh3cPR4="
    ...
    const res = await Promise.allSettled(promiseAll)
    res.forEach((re: any, index: number) => {
      if (re.status === "fulfilled") {
        // 成功的请求
        const { data } = re.value
        equipments.value[currentWindow.value] = equipmentIds[index]
        startPlay(data.content.url)
      } else {
        // 失败的请求
        ElMessage.info(`请求设备ID为 ${equipmentIds[index]} 视频流失败:${re.reason}`)
      }
    })
  } catch (error) {
...
const clickSwitch = async (command: string) => {
  ...
  if (showReplayPanel.value) {
    ...
    deviceId = equipments.value[currentWindow.value]
  }
  let _action = "0"
...
const changeTool = (tool: number) => {
  ...
  if (tool === 2) {
    if (videoUrl.value[currentWindow.value]) {
      ...
      equipments.value.splice(currentWindow.value, 1)
    }
  }
  // 刷新视频
  if (tool === 3) {
...
const queryReplay1 = async () => {
  ...
  try {
    ...
    const { response } = await GetPlaybackURL({
      deviceId: replayDevice.value.equipmentId, // "f3eae95f5045419aae6474a62850d79c",
      Playtype: "ws",
      beginTime: startTime,
      endTime: endTime
    })
...
const queryReplay = async () => {
  ...
  startTime = dayjs(replayDate.value).startOf("day").unix()
  endTime = dayjs(replayDate.value).endOf("day").unix()
  try {
    loading.value = true
    const { data } = await axios.get(
      `https://gwdm.510link.com/api/devices/dCameraInfoVServerApi/recordStart?uniqueCode=${replayDevice.value.equipmentId}&beginTime=${startTime}&endTime=${endTime}`,
      {
        headers: {
          "X-MAAAPI-ApiKey": "lOvA9FzlTYkOx4cg98UOTA==",
          "X-MAAAPI-ApiSecret": "pyNB9Ld11sn3ttidM+I9cUsOGSvYUep2G73lZh3cPR4="
    ...
    if (url) {
      ...
      ElMessage.success("播放成功，等待第一帧加载完成！")
      setTimeout(() => {
        replayVideoRef.value.replay(url, startHkFormat, endHkFormat)
      }, 100)
    } else {
      ElMessage.info("未能获取到视频流信息，请重新加载！")
    }
          ...
          <el-scrollbar height="100%">
            ...
            >
              <template #default="{ node, data }">
                ...
                >
                  {{ data.label }}（{{ data.ChildModel.length }}）
                </div>
                <div
                  v-else
                  class="child-node-label"
                  :class="{ active: equipments.includes(data.equipmentId) }"
                >
                  <div :class="{ offLine: data.status !== '在线' || data.eqtypename === '增强型' }">
                    <img
                      :src="$fun.getImg('researchManagement/monitor-icon1.png')"
                      alt=""
                    />
                    <span class="label-text"> {{ data.label }}({{ data.eqtypename }})</span>
                    <img
