...
const getData = async () => {
  ...
  if (response && response.length > 0) {
    ...
    response.forEach((item: any) => {
      yAxisData.push(item.time) // "2024-10-25 00:00:00",
      data.push(item.value)
    })
    //最大值
    max = data.reduce((a: any, b: any) => {
      return a > b ? a : b
    })
    ...
    chart2?.dispose && chart2.dispose() // 销毁实例
    chart2 = echarts.init(document.getElementById("chart2") as HTMLDivElement)
    option = {
