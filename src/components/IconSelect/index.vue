...
function loadIcons() {
  ...
  for (const icon in icons) {
    const iconName = icon.split("icons/svg/")[1].split(".svg")[0]
    if (!iconName.includes("查询")) {
...
<template>
  ...
  >
    ...
    >
      <template #reference>
        <div class="cursor-pointer sle text-[#999] absolute right-[10px] top-0 height-[32px] leading-[32px]">
          <i-ep-close @click="handleClear" />
        </div>
      </template>
